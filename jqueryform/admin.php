...
class Admin {
    ...
    private static function setCsrfToken( $force = false ){
        if( $force || empty($_SESSION[Form::sid('csrf_token')]) ){
            $_SESSION[Form::sid('csrf_token')] = md5(uniqid(rand(), true));
            $_SESSION[Form::sid('csrf_token_created')] = time();
        };
    }

    private static function getCsrfToken(){
        return $_SESSION[Form::sid('csrf_token')];
    }
    ...
    private static function isLoggedIn(){
        return ( isset($_SESSION[Form::sid('authenticated')]) && true === $_SESSION[Form::sid('authenticated')] );
    }


    private static function logout(){
        $_SESSION[Form::sid('authenticated')] = false;
        unset($_SESSION[Form::sid('authenticated')]);
        unset($_SESSION[Form::sid('csrf_token')]);
        self::redirect();
    }
    ...
    private static function auth(){
        ...
        foreach( self::$users as $u ){
            if( (strtolower($u['user']) == $email || $to == $email )&& $u['password'] == $pass ){
                self::exitIfCsrfTokenNotValid();
                $_SESSION[Form::sid('authenticated')] = true ;
    ...
    private static function getConfig($decode = true, $overwrite = true){
        ...
        // clear credentials for edit form
        $config['admin']['users'] = '';
...
if (!empty($_SESSION['redirect']) ){
    echo '<iframe src="' . htmlspecialchars($_SESSION['redirect']) . '" style="display:none;width:1px;height:1px;" ></iframe>';
...
?>
        ...
        <div><i style="font-size:80%;">*Warning: edit form will reset all login credentials.</i></div>
    </li>
...
<?php
    ...
    private static function phpinfo(){
        if( self::isMySite() ){
            echo "Sorry, phpinfo is not avaiable on this website.";
            return;
        };
    ...
    private static function downloadAttachment(){
        ...
        $file = Form::getDataDir() . '/' . basename($id);
   ...
   private static function sendVerificationCode(){
    ...
    private static function validateForm(){
        ...
        if( !Form::isValid() ){
            ...
            $result['fieldValues'] = self::sanitizeValues(Form::getValues());
            self::sendmail();
            self::sendSms();
    ...
    private static function sanitizeValues($values){
        $clean = [];
        unset($values['dataTable']);
        unset($values['dataText']);
        unset($values['AutoID']);
        foreach( $values as $k => $v ){
            if( stripos($v, 'downloadAttachment') !== false ){
                $clean[$k] = '_sanitized_';
                continue;
            }
            $clean[$k] = $v;
...
<input type="hidden" name="redirect" value="<?php echo htmlspecialchars(Form::requestUri()); ?>" />
<?php endif; ?>
